...
const debug = require("debug")("friend-facade")
...
class FriendsFacade {
  ...
  async deleteFriendV2(email: string): Promise<IFriend> {
    /* const user = await this.friendCollection.findOne({ email: email });
    const delUser = await this.friendCollection.deleteOne({ email: email }); */
    const status = await this.friendCollection.findOneAndDelete(
      { email },
      {  }
      /* const name = user.firstName;
      const user2 = {name, user.lastName, user.email, user.password}; */
    )
    return status.value;
  }
  ...
  convertObjectIdToId(friend: any) {
    const copy = { ...friend } //This is a clone, not a copy!
  ...
  async addFriend(friend: IFriend): Promise<{ id: String }> {
    ...
    const newFriend = this.friendCollection.insertOne(
      {
    ...
    const user = this.friendCollection.findOne({email:f.email});
  ...
  async editFriend(email: string, friend: IFriend): Promise<{ modifiedCount: number }> {
    ...
    const changedFriend = this.friendCollection.updateOne(
      { email: email },
      { $set:
        {
        firstName: f.firstName, 
  ...
  async deleteFriend(friendEmail: string): Promise<boolean> {
    const user = this.friendCollection.findOne({ email: friendEmail });
    const delUser = this.friendCollection.deleteOne({ email: friendEmail });
    return user;
  }
